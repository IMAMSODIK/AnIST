...
<div class="max-w-2xl">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
        <h3 class="text-lg font-semibold text-slate-800 dark:text-white mb-6">Edit Initiative</h3>
        <form method="POST" action="{{ route('initiatives.update', $initiative) }}" class="space-y-5">
            @csrf @method('PUT')
            <div>
                ...
                <select name="measurement_id" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm">
                    @foreach($measurements as $m)<option value="{{ $m->id }}" {{ old('measurement_id', $initiative->measurement_id) == $m->id ? 'selected' : '' }}>{{ $m->measurement }}</option>@endforeach
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Initiative Description</label>
                <textarea name="initiative" rows="4" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm focus:ring-2 focus:ring-indigo-200 outline-none">{{ old('initiative', $initiative->initiative) }}</textarea>
            </div>
            <div class="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-medium transition-colors">Update Initiative</button>
            </div>
        </form>
    </div>
</div>
