...
<div class="max-w-2xl">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
        <h3 class="text-lg font-semibold text-slate-800 dark:text-white mb-6">Edit Target</h3>
        <form method="POST" action="{{ route('targets.update', $target) }}" class="space-y-5">
            ...
            <div>
                ...
                <select name="measurement_id" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm">
                    @foreach($measurements as $m)<option value="{{ $m->id }}" {{ old('measurement_id', $target->measurement_id) == $m->id ? 'selected' : '' }}>{{ $m->measurement }}</option>@endforeach
                </select>
            </div>
            <div class="grid grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Year</label>
                    <input type="number" name="year" value="{{ old('year', $target->year) }}" min="2020" max="2050" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm">
                </div>
                <div>
                    ...
                    <select name="quarter" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm">
                        @foreach(['Q1','Q2','Q3','Q4'] as $q)<option value="{{ $q }}" {{ old('quarter', $target->quarter) == $q ? 'selected' : '' }}>{{ $q }}</option>@endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Target Value</label>
                    <input type="number" name="target" value="{{ old('target', $target->target) }}" step="0.01" min="0" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm">
                </div>
            </div>
            <div class="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-medium transition-colors">Update Target</button>
            </div>
        </form>
    </div>
</div>
